...
export default function Posts(data: PostProps): JSX.Element {
	...
	async function GetName(): void {
		const res = await axios.get('http://localhost:4200/profile/get?id='+data.userId);
		setName(res.data.result.name);
		console.log('this is the card response', res.data.result.name);
	}
	React.useEffect(() => {
