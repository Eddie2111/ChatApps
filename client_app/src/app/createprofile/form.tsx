						...
						count === 0 && (
							<MotionDiv direction='left'>
								...
								<Input
									...
									label='Name'
									placeholder='Name'
									onChange={(event: React.FormEvent<HTMLFormElement>) => setName(event.currentTarget.value)}
									defaultValue={dott?.name || ''}
								/>
								<Input
									className='w-full mt-2'
									label='Email'
									placeholder='Email'
									onChange={(event: React.FormEvent<HTMLFormElement>) => setEmail(event.currentTarget.value)}
									defaultValue={dott?.email || ''}
								/>
								<Input
									className='w-full mt-2'
									type='date'
									label='Date of Birth'
									placeholder=' '
									onChange={(event: React.FormEvent<HTMLFormElement>) => setDob(event.currentTarget.value)}
								/>
								<div className='flex flex-row justify-between mt-2'>
									<Button className='mt-2' onClick={() => setCount(count + 1)} color='primary' auto>
										{' '}
                    Next{' '}
									</Button>
								</div>
					...
					{count === 1 && (
						<MotionDiv direction='left'>
							...
							<Textarea
								...
								placeholder='I like to introduce myself...'
								className='max-w-xs'
								onChange={(event: React.FormEvent<HTMLFormElement>) => setAbout(event.currentTarget.value)}
							/>
							<Input
								className='w-full mt-2'
								label='location'
								placeholder='alpha, beta'
								onChange={(event: React.FormEvent<HTMLFormElement>) => setLocation(event.currentTarget.value)}
							/>
							<div className='flex flex-row justify-between mt-2'>
								<label className='mt-2'>
                  Contact <span className='text-sm text-gray-500'>(other social id)</span>
								</label>
								<Button
									className='mx-1 mt-[-2px]'
									onClick={() => setContactList([...contactlist, contactlist.length])}
									color='primary'
									variant='transparent'
									auto
								>
							...
							{contactlist.map((item, index) => (
								<div key={index} className='flex flex-row justify-between mt-2'>
									...
									>
										{' '}
										<RxCross2 className='text-red-600 text-2xl' />{' '}
									</Button>
								</div>
							))}
							<div className='flex flex-row justify-between mt-2'>
								<Button onClick={() => setCount(count - 1)} color='primary' auto>
									{' '}
                  Previous{' '}
								</Button>
								<Button onClick={() => setCount(count + 1)} color='primary' auto>
									...
									<div className='overflow-y-scroll'>
										<Combobox.Options>
											{filteredschoolsAndCollegesDhaka.map((person, index) => (
												<div
													key={index}
													className='bg-white border-t-1 border-black text-black dark:bg-gray-800 dark:text-white p-2'
												>
