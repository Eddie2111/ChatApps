...
export default function Form(): JSX.Element {
   ...
   */
     ...
     */
		...
		await axios
			.post<IResponseProps>('http://localhost:3100/signup', formdataset)
			.then((data) => {
				//console.log(data);
				call('Signup Successfull');
				localStorage.setItem('id', data.data.user.id);
				localStorage.setItem('email', formdataset['email']);
				localStorage.setItem('name', formdataset['name']);
			...
			<Card className='w-full md:w-[350px] my-auto p-2'>
				...
				<form onSubmit={HandleFormSubmit} className='w-full md:w-[330px] h-[320px]'>
					...
					<div className='flex flex-col justify-center'>
						<Button type='submit' color='secondary' className='w-full my-2'>
              Submit
						</Button>
						<Button onClick={() => router.push('/login')} color='primary' className='w-full'>
              Login
						</Button>
					</div>
				</form>
